...
def upload_repo():
    ...
    try:
        res = requests.post(
            f"{BASE_URL}/upload",
            files={"github_url": (None, REPO_URL)}
        )
        data = res.json()
...
def test_analysis(project_id):
    ...
    if issues and score is not None:
...
def test_chat(project_id):
    ...
    for q in queries:
        ...
        # VALIDATION
        if not answer or not mode or not sources:
            success = False
            print("❌ Chat missing fields")

        if "not found" in answer.lower():
            print("⚠️ No answer found (acceptable if truly missing)")
